...
### EXPLAIN ANALYZE
-> Filter: (count(0) = 2)  (actual time=0.032..0.032 rows=0 loops=1)
    -> Group aggregate: count(0)  (actual time=0.017..0.031 rows=6 loops=1)
        -> Filter: (comments.post_id in (375,376,377,388,389,390))  (cost=1.86 rows=8) (actual time=0.011..0.027 rows=8 loops=1)
            -> Index range scan on comments using idx_comments_post_id  (cost=1.86 rows=8) (actual time=0.010..0.025 rows=8 loops=1)
