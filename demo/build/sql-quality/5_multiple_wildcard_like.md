...
- **SQL File:** `5_multiple_wildcard_like.sql`
- **Cost:** 497.95
...
+- Table
   ...
   rows            4897
   filtered        20.99
   condition       ((`test`.`posts`.`title` like '%keyword%') or (`test`.`posts`.`content` like '%keyword%'))
```
## Analysis Detail
...
### EXPLAIN JSON
{"query_block":{"select_id":1,"cost_info":{"query_cost":"497.95"},"table":{"table_name":"posts","access_type":"ALL","rows_examined_per_scan":4897,"rows_produced_per_join":1027,"filtered":"20.99","cost_info":{"read_cost":"395.18","eval_cost":"102.77","prefix_cost":"497.95","data_read_per_join":"1M"},"used_columns":["id","user_id","title","content","status","view_count","created_at"],"attached_condition":"((`test`.`posts`.`title` like '%keyword%') or (`test`.`posts`.`content` like '%keyword%'))"}},"analyze_result":{"EXPLAIN":"-> Filter: ((posts.title like '%keyword%') or (posts.content like '%keyword%'))  (cost=498 rows=1028) (actual time=1.99..1.99 rows=0 loops=1)\n    -> Table scan on posts  (cost=498 rows=4897) (actual time=0.00158..1.1 rows=5000 loops=1)\n"}}

### EXPLAIN ANALYZE
{"EXPLAIN":"-> Filter: ((posts.title like '%keyword%') or (posts.content like '%keyword%'))  (cost=498 rows=1028) (actual time=1.99..1.99 rows=0 loops=1)\n    -> Table scan on posts  (cost=498 rows=4897) (actual time=0.00158..1.1 rows=5000 loops=1)\n"}
