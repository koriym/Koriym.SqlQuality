...
### EXPLAIN JSON
{"query_block":{"select_id":1,"cost_info":{"query_cost":"497.95"},"table":{"table_name":"posts","access_type":"ALL","rows_examined_per_scan":4897,"rows_produced_per_join":4897,"filtered":"100.00","cost_info":{"read_cost":"8.25","eval_cost":"489.70","prefix_cost":"497.95","data_read_per_join":"5M"},"used_columns":["id","user_id","title","content","status","view_count","created_at"],"attached_condition":"(cast(`test`.`posts`.`created_at` as date) = '2024-01-01')"}},"analyze_result":{"EXPLAIN":"-> Filter: (cast(posts.created_at as date) = '2024-01-01')  (cost=498 rows=4897) (actual time=1.3..1.3 rows=0 loops=1)\n    -> Table scan on posts  (cost=498 rows=4897) (actual time=0.00158..1.14 rows=5000 loops=1)\n"}}

### EXPLAIN ANALYZE
{"EXPLAIN":"-> Filter: (cast(posts.created_at as date) = '2024-01-01')  (cost=498 rows=4897) (actual time=1.3..1.3 rows=0 loops=1)\n    -> Table scan on posts  (cost=498 rows=4897) (actual time=0.00158..1.14 rows=5000 loops=1)\n"}
